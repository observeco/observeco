# I finally saw what my 39 AI agents and services were actually doing. Half of it was waste.

**Format:** X Article (~2,400 words, 4 embedded images)
...
I run 39 AI agents and services on one Mac Mini. They write code, manage my infrastructure, watch my servers, and talk to each other. For months, I assumed they were working fine.

They weren't—but I had no way to know.

[IMAGE: visual-card-fleet.png]
...
When I built ObserveCo and pointed it at my own fleet, here's what I found.

**28 out of 39 agents and services were alive. 11 were down.** A 28% failure rate I was completely blind to. Multiple agents marked "Guard: possible stale"—running, but with no guarantee they were producing anything useful.

**$112.45 in token spend I couldn't trace.** One agent (hermes-agent) was 91.5% of the total—$102.90. The cache hit rate was 3.9%, meaning 96% of every prompt was recomputed from scratch. Tool definitions, memory loops, heartbeat calls I never authorized.

[IMAGE: visual-card-cost.png]
...
## 3. What I did about it (the dogfooding part)
...
**Arize Phoenix** (10k★, OTel-native, 2.2M PyPI downloads/mo) is strong on LLM trace-level debugging—spans, evaluations, experiments. But it has no concept of agent runtime health. No circuit breakers, no pulse checks, no memory debt, no drift detection. It's LLM-trace-centric, not agent-runtime-centric.
...
**OpenLIT** (2.5k★, Apache 2.0) offers GPU monitoring and 60+ integrations, all free forever. Broad but shallow on agent health. No compression, no memory hygiene.
...
| **Auto-Restart** | Detects crashes and restarts automatically (v0.3) |
| **Push Alerts** | Telegram when agents go down |
| **Circuit Breaker** | Stops cascading failures |

[IMAGE: visual-card-heal.png]

**The observation banners are the most important part.** When the dashboard flags "79,826 tokens—critically bloated," it tells you exactly what to run. v1.1 (~2 weeks) will execute it automatically.
...
*I run 39 AI agents and services on a Mac Mini. This is what I built to keep them honest.*