...
ObserveCo is an open-source observability platform for local AI agents. This plugin exports telemetry from every Hermes conversation to an ObserveCo OTEL listener, which writes to a local SQLite database that powers the ObserveCo dashboard.
...
│    │ pre_gateway_dispatch → msg routed │     │
│    └──────────────────────────────────┘     │
│                    │                        │
│              OTLP/HTTP POST                 │
│                    ▼                        │
...
| `hermes.tool_result` | `post_tool_call` only — summary of result (e.g. `ok (1234 chars)`, `error: ...`) |
...
## Setting Up the OTEL Listener

The OTEL listener is a lightweight FastAPI server that accepts OTLP/HTTP spans and writes them to `pulse.db`. It ships with ObserveCo.

```bash
# Install ObserveCo
pip install observeco

# Start the listener (daemon mode)
observeco otel listen start --port 4318

# Check status
observeco otel listen status

# Stop
observeco otel listen stop

# Run once (foreground, for testing)
observeco otel listen --once
```

The listener auto-deduplicates spans on exporter retries (in-memory dedup set, 50K span window).

## Viewing the Dashboard

```bash
observeco dashboard --port 8123
```

Open `http://localhost:8123` in your browser. The dashboard shows:

- **Token Analytics** — token usage over time, broken down by agent and model
- **Agent Health** — pulse status, error rates, uptime
- **Session Activity** — active sessions, conversation counts
- **Cost Tracking** — per-agent and per-model cost breakdowns
...
1. **Is the plugin enabled?**
   ...
   ```
   Should show `observability/observeco` as enabled.
...
4. **Check the gateway log for plugin registration:**
   ...
   ```
   Should show: `ObserveCo plugin registered — will export spans to http://127.0.0.1:4318`
...
The plugin is fire-and-forget with a 5-second timeout. Failures are logged at DEBUG level. To see them:
